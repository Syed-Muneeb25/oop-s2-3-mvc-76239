...
namespace VgcCollege.Web.Data
{
    public static class DataSeeder
    {
        public static async Task SeedAsync(IServiceProvider serviceProvider)
        {
            ...
            {
                Name = "Dublin Campus",
                Address = "8 Belvedere Place, Dublin 2"
            };

            var cork = new Branch
            {
                Name = "Cork Campus",
                Address = "123 Cork Street, Cork"
            };

            var galway = new Branch
            {
                Name = "Galway Campus",
                Address = "525 Mount Street Rd, Galway"
            };
            ...
            if (f1User == null || f2User == null)
            {
                throw new Exception("Faculty users were not created correctly.");
            }
            ...
            if (s1User == null || s2User == null || s3User == null)
            {
                throw new Exception("Student users were not created correctly.");
            }
            ...
            if (existingUser != null)
            {
                if (!await userManager.IsInRoleAsync(existingUser, role))
                {
                    ...
                    if (!addRoleResult.Succeeded)
                    {
                        ...
                        throw new Exception($"Failed to add existing user {email} to role {role}: {errors}");
                    }
                }
